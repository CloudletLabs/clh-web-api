module.exports = {
    checkDefaultTestData: function (models) {
        helper = this;
        helper._checkCount(helper, models.userRole, {}, helper._createDefaultUserRoles, function (results) {
            helper._checkCount(helper, models.user, results, helper._createDefaultUsers, function (results) {
                helper._checkCount(helper, models.news, results, helper._createDefaultNews, function (results) {
    ...
    _getModelName: function (model) {
        return model.modelName.replace(/\b\w/g, function (l) {
                return l.toLowerCase()
            }) + 's';
    },
    _checkCount: function (helper, model, results, createDefaultCallback, next) {
        model.count(function (err, count) {
            if (err) throw (err);
            if (count <= 0) {
                createDefaultCallback(helper, model, results, next);
            } else {
    ...
    _getExisting: function (helper, model, results, next) {
        model.find(function (err, result) {
            if (err) throw err;
            var modelName = helper._getModelName(model);
    ...
    _createDefaultUserRoles: function (helper, UserRole, results, next) {
        var roles = [
            {roleId: "ADMIN", displayName: "Administrator"},
            {roleId: "USER", displayName: "User"}
        ];
        helper._createDefaultData(helper, UserRole, results, roles, next);
    },
    _createDefaultUsers: function (helper, User, results, next) {
        var users = [
            {
                username: 'admin',
        ...
        helper._createDefaultData(helper, User, results, users, next);
    },
    _createDefaultNews: function (helper, News, results, next) {
        var news = [
            {
                slug: 'hello-world',
        ...
        helper._createDefaultData(helper, News, results, news, next);
    }
};